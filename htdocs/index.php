...
if ($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    if (!empty($phone) && !empty($password)) {
        // 1. መጀመሪያ ስልክ ቁጥሩ በዳታቤዝ ውስጥ መኖሩን እንፈትሻለን
        $stmt = $pdo->prepare("SELECT * FROM users WHERE phone = ?");
        $stmt->execute([$phone]);
        $user = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$user) {
            // ስልክ ቁጥሩ በዳታቤዝ ውስጥ ከሌለ
            $error = "This account does not exist! Please sign up first.";
        } else {
            // ስልክ ቁጥሩ ካለ ፓስወርዱ ትክክል መሆኑን እናረጋግጣለን
            ...
            <form action="index.php" method="POST" autocomplete="off" class="space-y-4">
                <div>
                    ...
                    <div class="flex items-center bg-slate-900 border border-slate-800 rounded-xl overflow-hidden">
                        ...
                        <input type="text" name="phone" value="<?php echo htmlspecialchars($saved_phone); ?>" placeholder="912345678" required class="w-full bg-transparent px-3 py-2.5 text-xs text-slate-200 outline-none">
                    </div>
                </div>
